...
<div dir="rtl">
جهت مشاهده  تمام ویژگی‌های ES6 به این لینک سر بزنید
<a href="http://www.ecma-international.org/ecma-262/6.0/">ES2015 standard</a>.
</div>

<div dir="rtl">
لیست ویژگی های جدید ES6:
</div>

- [ES2015 (ES6)<sup>credit</sup>](#es2015-es6supcreditsup)
...
Arrows فانکشن کوتاه شده‌ای هستش که بصورت علامت `=>` استفاده میشود. این ویژگی بسیار عالی در سایر زبانهای مانند C#, Java, CaffeeScript مشابهت دارد.
در فانکشن کوتاه شده میتوانید بصورت خلاصه (`Expression bodies`) یا دارای بدنه بلاکی (`Statement bodies`) بنویسید. همچنین `this` در Arrow فانکشن به آبجکت بالای خود اشاره میکند نه فانکشن جاری و رفتاری مانند فانکشن های معمولی ندارد به مثال  `Lexical this` توجه کنید.
...
// درصورت برگرداندن آبجکت باید درون پرانتز قرار بگیرد که با بدنه فانکشن اشتباه نشود
...
var bob = {
  ...
  _friends: [],
  printFriends() {
    // میکند نـه به فانکشن جاری bob اشاره به ابجکت this
...
اطلاعات بیشتر: [MDN Arrow Functions](https://developer.mozilla.org/en/docs/Web/JavaScript/Reference/Functions/Arrow_functions)
...
با به روزرسانی جاوااسکریپت در ES6 تمام ویژگی های OOP راحت تر نسبت به حالت prototype  قابل توسعه می باشد. و تمام ویژگی‌های `prototype-based` `inheritance`, `super calls`, `instance`, `static methods` و `constructors` را از آن بهره می‌برید.

توجه داشته باشید که اضافه شدن ویژگی `class` در نهایت تبدیل به همان `prototype` میشود
</div>

```JavaScript
class SkinnedMesh extends THREE.Mesh { // ارث پذیری
  constructor(geometry, materials) { // Constructor
    super(geometry, materials); // فراخوانی کلاس والد
  ...
  update(camera) { // Method
    //...
    super.update(); // فراخوانی متد کلاس والد
  }
  get boneCount() { // Property
    return this.bones.length;
  }
  set matrixType(matrixType) {
    this.idMatrix = SkinnedMesh[matrixType]();
  }
  static defaultMatrix() { // Static Method
    return new THREE.Matrix4();
  }
}
```

اطلاعات بیشتر: [MDN Classes](https://developer.mozilla.org/en/docs/Web/JavaScript/Reference/Classes)
...
ارتقابخشی تعریف آبجکت در ES6 که ویژگی های خوبی به آن اضافه شده مانند: 
ارث‌بری از ابجکت دیگر، مقدار دهی‌ها، تعریف متد،فراخوانی آبجکت والد، تعریف کلید بواسطه سایر متغییرها.

این نوع ویژگی‌های تعریف آبجکت بسیار مشابه تعاریف شی-گرای میباشد.

</div>

```JavaScript
const handler = "Enhanced Property Object"

var obj = {
    //ارث بری
    __proto__: theProtoObj,
    // ساده سازی اضافه کردن آبجکت هم نام
    ...
    // نام گذاری کلید ابجکت بصورت داینامیک در آبجکت
    [ 'prop_' + (() => 42)() ]: 42 // prop_42: 42
};
```

اطلاعات بیشتر: [MDN Grammar and types: Object literals](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Grammar_and_types#Object_literals)
